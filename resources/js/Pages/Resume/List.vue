                ...
                <div class="flex">
                    ...
                    <div>
                        <div class="grid gap-4 grid-cols-2">
                            <div v-for="resume in $page.props.resumes">
                                <Link :href="route('resumes.show', resume.id)" class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                                    <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ resume.job }}</h5>
                                    <div v-if="resume.text" class="font-normal text-gray-700 dark:text-gray-400">
                                        {{ truncate(resume.text, { length: 160 }) }}
                                        <ul class="list-none mt-3">
                                            <li>
                                                <strong>Created at:</strong> {{ moment.tz(resume.created_at, 'UTC').tz(moment.tz.guess()).calendar() }}
                                            </li>
                                        </ul>
                                    </div>
                                </Link>
